...
common_geno=subset(common_geno, genotype %in% common.geno)

#outputting boxplots of common traits for common genotypes across environments
pdf('./results/Set_RIL_field_common_traits_common_genos_summary_boxplots.pdf', height = 12, width = 14)
ggplot(common_geno, aes(x=year, y=data, group=interaction(year,ordered(treatment, levels=c('wet','dry','thick','sparse'))), fill=treatment))+
  geom_boxplot()+
...
#PCA with plant harvest traits as DAS and without LMA and R:V
pca.matrix2=common.wide.complete[,c(5:10,16:23)]
colnames(pca.matrix2)
...
png("./results/PC12_DAS.png", width=900, height=700)
ggbiplot(PCA_multiyear, choices=c(1,2),labels.size = 2,obs.scale=1,varname.adjust = 1, var.scale=1, groups=env, ellipse=TRUE)+xlim(c(-6,11))+ylim(c(-4,6.5))
dev.off()
png("./results/PC13_DAS.png", width=900, height=700)
ggbiplot(PCA_multiyear, choices=c(1,3),labels.size = 2,obs.scale=1,varname.adjust = 1, var.scale=1, groups=env, ellipse=TRUE)+xlim(c(-6,11))+ylim(c(-4,5))
dev.off()
png("./results/PC23_DAS.png", width=900, height=700)
ggbiplot(PCA_multiyear, choices=c(2,3),labels.size = 2,obs.scale=1,varname.adjust = 1, var.scale=1, groups=env, ellipse=TRUE)+xlim(c(-4,6.5))+ylim(c(-4,5))
dev.off()
...
mfa.data=common.wide.complete[,c(1,4,5:8,16:22,9,23,10)]
colnames(mfa.data)
try=MFA(mfa.data, group = c(2,4,7,3), type = c('n','s','s','s'), 
        name.group = c('environment','leaf','biomass','growth'),
        num.group.sup=c(1))
