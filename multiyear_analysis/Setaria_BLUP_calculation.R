...
parent.data=all_stack1[which(all_stack1$genotype %in% c('A10','B100')),]
...
parent.blup<-c()
pdf('/acct/employee/lsipusr/Desktop/parent_blups.pdf')
...
exp.combos=c("density_2013","drought_2014","density_2014","drought_2015")
parent.blup.all<-c()
rils.blups<-c()
for(d in 1:length(exp.combos)){
  temp=all[all$environment == exp.combos[d],]
  traits=unique(temp$trait)
    pdf(paste('/acct/employee/lsipusr/Desktop/',exp.combos[d],'_blups.pdf', sep=""))
